  ...
  return (
    <View style={styles.container}>
      {TABS.map((tab) => {
        ...
        return (
          <TouchableOpacity key={tab.key} style={styles.tabButton} onPress={() => onTabPress(tab.key)} activeOpacity={0.7}>
            <Ionicons name={tab.icon} size={24} color={isActive ? '#1A1A1A' : '#9B9B9B'} />
          </TouchableOpacity>
        );
      })}
    </View>
